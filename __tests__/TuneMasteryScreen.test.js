...
import { useTuneMasteryData } from "../src/hooks/useTuneMasteryData";
...
jest.mock("../src/hooks/useSelectionEngine", () => ({
  useSelectionEngine: jest.fn(() => ({
    ...
    getTuneName: jest.fn((id) => (id === "tune1" ? "Test Tune" : "Unknown")),
    isLearningPick: true,
...
jest.mock("../src/screens/TuneMastery/components/PracticePanel", () => {
  ...
  return function MockPracticePanel({ tuneName, tuneKey, onCancel }) {
    return (
...
describe("TuneMasteryScreen", () => {
  beforeEach(() => {
    jest.clearAllMocks();
    useTuneMasteryData.mockReturnValue(createMockHookReturn());
  });

  describe("rendering", () => {
    it("renders loading state", () => {
      useTuneMasteryData.mockReturnValue(
    ...
    it("renders error state", () => {
      useTuneMasteryData.mockReturnValue(
        createMockHookReturn({ error: { message: "Test error" } }),
      );
  ...
  describe("add tune", () => {
    ...
    it("calls addTune with input value", async () => {
      ...
      useTuneMasteryData.mockReturnValue(
        createMockHookReturn({ addTune: mockAddTune }),
      );
