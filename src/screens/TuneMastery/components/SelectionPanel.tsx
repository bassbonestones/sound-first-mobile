...
export interface SelectionPanelProps {
  tunes: Array<{ id: string; name: string }>;
  selectedTuneId: string | null;
  selectedKey: MusicalKey | null;
  onSelectTune: (tuneId: string | null) => void;
...
const SelectionPanel = React.memo(function SelectionPanel({
  ...
  const tuneOptions = useMemo<TuneOption[]>(
  ...
  const renderTuneItem = useCallback(
    ({ item }: ListRenderItemInfo<TuneOption>) => (
      <TouchableOpacity
        style={[
          styles.pickerItem,
      ...
      >
        ...
        {selectedTuneId === item.id && <Text style={styles.checkmark}>✓</Text>}
      </TouchableOpacity>
    ),
    [selectedTuneId, onSelectTune, closeTunePicker],
  );

  const renderKeyItem = useCallback(
    ({ item }: ListRenderItemInfo<KeyOption>) => (
      <TouchableOpacity
        style={[
      ...
      >
        <Text
          style={[
            ...
            selectedKey === item.key && styles.keyPickerItemTextSelected,
          ]}
        >
          {item.label}
        </Text>
    ...
    [selectedKey, onSelectKey, closeKeyPicker],
  );
