...
export default function FeelThePulseExercise({ config, mastery, onComplete, onProgress }) {
  ...
  const getDeviationFromBeat = useCallback((tapTime) => {
      ...
      if (beatCount < listeningBeats) {
        ...
        if (success) {
          ...
          if (newRounds >= masteryRounds) {
            setTimeout(() => onComplete?.({ success: true, roundsCompleted: newRounds }), 1500);
          } else {
            onProgress?.({ roundsCompleted: newRounds, masteryRequired: masteryRounds });
            setTimeout(() => { if (!unmountedRef.current) startRound(); }, 2000);
          }
        } else {
          onProgress?.({ roundsCompleted, masteryRequired: masteryRounds });
          setTimeout(() => { if (!unmountedRef.current) startRound(); }, 2500);
        }
      }
    }, beatIntervalMs);
  }, [listeningBeats, silentBeats, beatIntervalMs, masteryRounds, roundsCompleted, playBeat, animatePulse, evaluateRound, onComplete, onProgress]);
      ...
      <View style={styles.container}>
        <View style={styles.introContainer}>
          ...
          <View style={styles.instructionBox}>
            ...
            <Text style={styles.simpleInstruction}>
              Then tap {silentBeats} times to continue the beat without the metronome.
            </Text>
          </View>
  ...
  return (
    <View style={styles.container}>
      <View style={styles.progressBar}>
        <View style={[styles.progressFill, { width: `${(roundsCompleted / masteryRounds) * 100}%` }]} />
      </View>
      <Text style={styles.streakText}>Round {roundsCompleted + 1} / {masteryRounds}</Text>

      <View style={[
        styles.phaseBanner,
        phase === PHASE_SILENT && styles.phaseBannerSilent,
        phase === PHASE_REVEAL && (roundResult === "success" ? styles.phaseBannerSuccess : styles.phaseBannerFail),
      ]}>
        <Text style={styles.phaseBannerText}>
          {phase === PHASE_LISTENING && `Listen... ${currentBeat} / ${listeningBeats}`}
          {phase === PHASE_SILENT && "TAP!"}
          {phase === PHASE_REVEAL && (roundResult === "success" ? "Nice!" : "Try again...")}
        </Text>
      </View>

      <TouchableOpacity style={styles.tapArea} onPress={handleTap} activeOpacity={0.8} disabled={phase !== PHASE_SILENT}>
        <Animated.View
          style={[
            styles.pulseCircle,
            ...
            phase === PHASE_REVEAL && (roundResult === "success" ? styles.circleSuccess : styles.circleFail),
          ]}
        >
          ...
          {phase === PHASE_REVEAL && (
            <>
              <Text style={styles.circleIcon}>{roundResult === "success" ? "🎉" : "💪"}</Text>
              <Text style={styles.circleMainText}>{roundResult === "success" ? "Nice!" : "Close!"}</Text>
            </>
          )}
        </Animated.View>
      </TouchableOpacity>
      ...
      {phase === PHASE_SILENT && (
        <Animated.View style={[styles.feedback, { opacity: feedbackOpacity }]}>
          <Text style={[styles.feedbackText, { color: getFeedbackColor() }]}>
            {lastTapFeedback === "perfect" ? "Perfect!" : lastTapFeedback === "good" ? "Good!" : lastTapFeedback === "off" ? "Off" : ""}
          </Text>
        </Animated.View>
      )}
...
const styles = StyleSheet.create({
  container: { flex: 1, padding: 20, alignItems: "center", backgroundColor: "#1a1a1a" },
  
  // Intro styles
  introContainer: { flex: 1, justifyContent: "center", alignItems: "center", paddingHorizontal: 20 },
  introIcon: { fontSize: 64, marginBottom: 16 },
  introTitle: { fontSize: 28, fontWeight: "bold", color: "#fff", marginBottom: 24 },
  instructionBox: { backgroundColor: "#2a2a2a", borderRadius: 16, padding: 24, width: "100%", marginBottom: 32 },
  simpleInstruction: { fontSize: 18, color: "#fff", lineHeight: 28, marginBottom: 12, textAlign: "center" },
  startButton: { backgroundColor: "#4CAF50", paddingVertical: 16, paddingHorizontal: 48, borderRadius: 30 },
  startButtonText: { fontSize: 18, fontWeight: "bold", color: "#fff" },
  bpmHint: { fontSize: 14, color: "#666", marginTop: 16 },

  // Progress
  progressBar: { width: "100%", height: 8, backgroundColor: "#333", borderRadius: 4, marginBottom: 8, overflow: "hidden" },
  progressFill: { height: "100%", backgroundColor: "#4CAF50", borderRadius: 4 },
  streakText: { fontSize: 14, color: "#888", marginBottom: 12 },

  // Phase banner
  phaseBanner: { backgroundColor: "#2196F3", paddingVertical: 12, paddingHorizontal: 20, borderRadius: 12, marginBottom: 20, width: "100%" },
  phaseBannerSilent: { backgroundColor: "#9C27B0" },
  phaseBannerSuccess: { backgroundColor: "#4CAF50" },
  phaseBannerFail: { backgroundColor: "#FF5722" },
  phaseBannerText: { fontSize: 18, fontWeight: "600", color: "#fff", textAlign: "center" },

  // Tap area
  tapArea: { flex: 1, justifyContent: "center", alignItems: "center", width: "100%" },
  pulseCircle: { width: 200, height: 200, borderRadius: 100, justifyContent: "center", alignItems: "center", elevation: 8, shadowColor: "#000", shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.3, shadowRadius: 8 },
