...
export default function RangeExpansionExercise({
  ...
  const analyzeContour = useCallback(() => {
    ...
    console.log(
      `[RangeExpansion] First note ${midiToNote(notes[0])} vs anchor ${anchorNote} (pitch class distance: ${pitchClassDistance})`,
    );
    ...
    const detectedIntervals = notes
      .slice(0, pattern.intervals.length)
      .map((midi, i) => {
    ...
    console.log(
      `[RangeExpansion] Detected intervals: [${detectedIntervals.join(", ")}]`,
    );
    console.log(
      `[RangeExpansion] Expected intervals: [${pattern.intervals.join(", ")}]`,
    );
    ...
    for (let i = 0; i < pattern.intervals.length; i++) {
      ...
      if (diff >= CONTOUR_TOLERANCE_SEMITONES) {
        console.log(
          `[RangeExpansion] Interval ${i} mismatch: expected ${expected}, got ${detected} (diff=${diff})`,
        );
        allMatch = false;
        break;
      }
      ...
      <View style={styles.container}>
        ...
        <View style={styles.infoBox}>
          <Text style={styles.infoLabel}>
            Current {direction === "up" ? "high" : "low"} note:
          </Text>
  ...
  // SING PHASE
  if (phase === PHASE.SING) {
    const detectedNote = currentPitch?.midiNote
      ? midiToNoteInContext(Math.round(currentPitch.midiNote), anchorNote)
      : null;
      ...
      <View style={styles.container}>
        ...
        <View style={styles.recordingIndicator}>
          <View
            style={[
              styles.recordingDot,
              isSounding && styles.recordingDotActive,
            ]}
          />
        ...
        <Text style={styles.hintText}>
          {notesRecorded > 0
            ? `Notes recorded: ${notesRecorded}`
            : "Start singing the pattern"}
        </Text>
    ...
    return (
      <View style={styles.container}>
        <Text style={styles.phaseTitle}>
          {overallSuccess ? "✅ Great!" : "🔄 Try Again"}
        </Text>
        ...
        <View style={styles.progressRow}>
          <Text style={styles.progressText}>
            Progress: {successfulRounds + (overallSuccess ? 1 : 0)} /{" "}
            {masteryThreshold}
          </Text>
        </View>
