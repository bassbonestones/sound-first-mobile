...
const PIANO_KEYS_SHARP: PianoKeyData[] = [
...
const PIANO_KEYS_FLAT: PianoKeyData[] = [
  { note: "C4", isBlack: false, label: "C" },
...
const BLACK_KEY_INDICES_SHARP: Record<string, number> = {
  "C#4": 0,
  "D#4": 1,
  "F#4": 3,
  "G#4": 4,
...
const BLACK_KEY_INDICES_FLAT: Record<string, number> = {
  Db4: 0,
  Eb4: 1,
  Gb4: 3,
  Ab4: 4,
...
export interface MiniKeyboardProps {
  /** Notes to highlight in green */
  highlightNotes?: string[];
  /** Note to highlight as flat (blue) */
  highlightFlat?: string | null;
  /** Note to highlight as sharp (orange) */
  highlightSharp?: string | null;
  /** Note to show as "skip" */
  skippedNote?: string | null;
  /** Callback when a key is pressed */
  onKeyPress?: (note: string) => void;
  /** Whether keys are pressable */
  interactive?: boolean;
  /** Use flat note names (Db, Eb) vs sharp (C#, D#) */
  useFlatNames?: boolean;
}
...
const MiniKeyboard = React.memo(function MiniKeyboard({
  ...
  const renderWhiteKey = (key: PianoKeyData) => {
  ...
  const renderBlackKey = (key: PianoKeyData) => {
    const isHighlighted = highlightNotes.includes(key.note);
    const isFlatHighlight = highlightFlat === key.note;
    const isSharpHighlight = highlightSharp === key.note;
