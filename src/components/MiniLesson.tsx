...
/**
 * Step types for mini-lesson
 */
type StepType = "LISTEN" | "EXPLAIN" | "VISUAL" | "TRY_IT" | "QUIZ";

/**
 * Lesson step data from API
 */
interface LessonStep {
  ...
  audio_url?: string;
  visual_url?: string;
  quiz_options?: string; // JSON string of options
  quiz_answer?: string;
}
...
export interface MiniLessonProps {
  /** Capability ID to load lesson for */
  capabilityId: string | number;
  /** Callback when lesson is completed */
  onComplete: (passed: boolean) => void;
  /** Callback to cancel/skip the lesson */
  onCancel?: () => void;
  /** User ID for tracking quiz results */
  userId?: number;
}

// Step type icons for mini-lesson
const LESSON_STEP_ICONS: Record<StepType, string> = {
  LISTEN: "🎧",
  EXPLAIN: "📖",
  VISUAL: "👁️",
  TRY_IT: "🎯",
...
const LESSON_STEP_LABELS: Record<StepType, string> = {
  LISTEN: "Listen",
  EXPLAIN: "Learn",
  VISUAL: "See It",
  TRY_IT: "Try It",
...
export default function MiniLesson({
  ...
  const handleQuizAnswer = async (answer: string): Promise<void> => {
    if (!lesson) return;
  ...
  const quizOptions: string[] = currentStep.quiz_options
    ? JSON.parse(currentStep.quiz_options)
    : [];
...
interface Styles {
  ...
  headerDomain: TextStyle;
  progressContainer: ViewStyle;
  progressBar: ViewStyle;
  stepIndicators: ViewStyle;
  stepDot: ViewStyle;
  stepDotActive: ViewStyle;
  stepDotCompleted: ViewStyle;
  stepDotIcon: TextStyle;
  contentContainer: ViewStyle;
  stepHeader: ViewStyle;
  stepIcon: TextStyle;
  stepLabel: TextStyle;
  stepInstruction: TextStyle;
  mediaContainer: ViewStyle;
  playButton: ViewStyle;
  playButtonText: TextStyle;
  mediaHint: TextStyle;
  notationPlaceholder: ViewStyle;
  notationPlaceholderText: TextStyle;
  explanationContainer: ViewStyle;
  explanationText: TextStyle;
  tryItContainer: ViewStyle;
  tryItPrompt: TextStyle;
  tryItHint: TextStyle;
  quizContainer: ViewStyle;
  quizOption: ViewStyle;
  quizOptionSelected: ViewStyle;
  quizOptionText: TextStyle;
  quizResult: ViewStyle;
  quizResultIcon: TextStyle;
  quizResultText: TextStyle;
