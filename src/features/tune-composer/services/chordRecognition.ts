 ...
 *
 * Note: For MusicXML generation, import generateChordPreviewMusicXml from
 * chordMusicXmlGenerator.ts or the services barrel export.
 */
...
export const ROOT_TO_SEMITONE: Record<ChordRoot, number> = {
  C: 0,
  "B#": 0,
  "C#": 1,
  Db: 1,
...
/**
 * Alteration intervals for extensions added via parentheses.
 * Maps alteration strings to semitone intervals from the root.
 */
export const ALTERATION_INTERVALS: Record<string, number> = {
  // 9ths (2nd + octave = 14, but altered)
  "9": 14, // natural 9
  b9: 13, // flat 9
  "#9": 15, // sharp 9
  // 11ths (4th + octave = 17, but altered)
  "11": 17, // natural 11
  "#11": 18, // sharp 11
  // 13ths (6th + octave = 21, but altered)
  "13": 21, // natural 13
  b13: 20, // flat 13
  // 5th alterations (also usable in parens)
  b5: 6,
  "#5": 8,
};

/**
 * Map from alteration to the natural tone it replaces.
 * When applying an alteration, we remove the natural version.
 */
export const ALTERATION_REPLACES: Record<string, number> = {
  b5: 7, // b5 replaces natural 5
  "#5": 7, // #5 replaces natural 5
  b9: 14, // b9 replaces natural 9
  "#9": 14, // #9 replaces natural 9
  "#11": 17, // #11 replaces natural 11
  b13: 21, // b13 replaces natural 13
};
...
export function spellChord(symbol: string, rootOctave: number = 60): number[] {
  ...
  const noteSet = new Set(intervals.map((interval) => baseNote + interval));

  // Apply alterations (e.g., "b9", "#11", "13")
  // Alterations can replace natural tones (e.g., b5 replaces natural 5)
    ...
    for (const alt of alterations) {
      ...
      if (altInterval !== undefined) {
        ...
        if (replacedInterval !== undefined) {
          noteSet.delete(baseNote + replacedInterval);
        }
        // Add the altered tone
        noteSet.add(baseNote + altInterval);
      }
    }
  }
...
// Note: MusicXML generation (generateChordPreviewMusicXml) has been
// extracted to chordMusicXmlGenerator.ts and re-exported above.
